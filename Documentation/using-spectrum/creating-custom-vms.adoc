...
Custom VMs can be configured on
xref:../development/persistent-storage.adoc[persistent storage].  VMs
configurations are directories under a dedicated parent directory, and
...
inter-guest networking have locally administered addresses (LAAs).
...
network interface named `eth0` has the MAC address `02:00:00:00:A5:46`
...
directory on the host, with the tag "host".  The VM cannot write
